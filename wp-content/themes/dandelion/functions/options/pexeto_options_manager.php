...
class PexetoOptionsManager{
	...
	var $pexeto_options_url='';
	var $pexeto_functions_url='';
	 ...
	 * @param $functions_url the URL of the functions directory
	 * @param $uploads_url the URL of the uploads directory
	 */
	function PexetoOptionsManager($themename, $options_url, $functions_url, $uploads_url, $version){
		$this->themename=$themename;
		$this->pexeto_options_url=$options_url;
		$this->pexeto_functions_url=$functions_url;
	...
	function print_heading($heading_text){
		...
		echo '</ul></div></div><div id="content"><div id="header"><h3 id="theme_name">'.$this->themename;
		if(!isset($_GET['pexeto_update'])){
			echo ' v.'.$this->pexeto_version;
		}
		echo '</h3><a class="more-button" href="http://themeforest.net/user/pexeto/portfolio"></a></div><div id="options_container">';
	}
	...
	function print_footer(){
		echo '</div></div><div id="pexeto-footer"><div id="follow-pexeto"> 
			 ...
			 <li><a href="http://twitter.com/pexeto" title="Pexeto on Twitter"><img src="'.$this->pexeto_options_url.'images/twitter.png" /></a></li>
			 <li><a href="http://themeforest.net/user/pexeto/follow" title="Follow my work on ThemeForest"><img src="'.$this->pexeto_options_url.'images/tf.png" /></a></li>
			 <li><a href="http://pexeto.com"><img src="'.$this->pexeto_options_url.'images/pex.png" title="Visit my website" /></a></li>
	...
	function print_upload($value){
		...
		echo '<script type="text/javascript">jQuery(document).ready(function($){
			pexetoOptions.loadUploader(jQuery("div#'.$value['id'].'_button"), "'.$this->pexeto_functions_url.'upload-handler.php", "'.$this->pexeto_uploads_url.'");
		});</script>'; 
	...
	function print_custom($value){
		...
		foreach($value['fields'] as $field){
			...
			switch($field['type']){
				...
				case 'upload':
					...
					echo '<script type="text/javascript">jQuery(document).ready(function($){
								pexetoOptions.loadUploader(jQuery("div#'.$field['id'].'_button"), "'.$this->pexeto_functions_url.'upload-handler.php", "'.$this->pexeto_uploads_url.'");
						});</script>';
					$preview=$field['id'];
				break;
				case 'textarea':
		...
		echo '<script type="text/javascript">
			jQuery(document).ready(function($){
				pexetoOptions.setCustomFieldsFunc("'.$value['id'].'", ["'.$idsString.'"], ["'.$namesString.'"], "'.$value['preview'].'", "'.PEXETO_TIMTHUMB_URL.'");
			});
		</script>';
